...
function UsageRow({ label, totals, since }: { label: string; totals: Totals; since: string }) {
  const cost = totals.costUsd === null ? null
    : totals.costUsd > 0 && totals.costUsd < 0.000001 ? '<$0.000001' : dollars.format(totals.costUsd);
  return <section className="usage-period" aria-label={`${label} usage`}>
    ...
    {totals.unpricedTokens > 0 && <p className="field-help">{tokens.format(totals.unpricedTokens)} tokens unpriced{cost !== null ? ' - excluded from cost' : ' - no matching model price was recorded'}. Price changes apply only to new requests.</p>}
...
export function UsageTotals({ usage }: { usage: UsageSummary | null }) {
  return <div className="usage-summary">
    {!usage ? <p className="field-help">Loading recorded usage...</p> : <>
      ...
      <p className="field-help">Includes live reported usage; saved when requests end. Session = this MCM server run.</p>
